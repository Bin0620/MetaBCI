...
class NeuroPy(BaseAmplifier):
    """NeuroSky MindWave 设备支持类
    - author: Sahil Singh (原始代码), 改编为 MetaBCI 放大器接口
    - 功能: 通过串口获取 NeuroSky MindWave 头戴设备的脑电数据
    """
    ...
    def start(self):
        ...
        self._exit.clear()
        self._t_loop = threading.Thread(target=self._inner_loop, name="neuropy_loop")
        self._t_loop.start()
    ...
    def _inner_loop(self):
        ...
        while not self._exit.is_set():
            try:
                # 查找数据包头
                p1 = self.serial_conn.read(1).hex()
                p2 = self.serial_conn.read(1).hex()
                while p1 != 'aa' or p2 != 'aa':
                    p1 = p2
                    p2 = self.serial_conn.read(1).hex()

                # 解析有效数据包
                payload_length = int(self.serial_conn.read(1).hex(), 16)
                payload = [self.serial_conn.read(1).hex() for _ in range(payload_length)]
                checksum = ~sum(int(x, 16) for x in payload) & 0xFF

                if checksum == int(self.serial_conn.read(1).hex(), 16):
                    self._parse_payload(payload)

            except Exception as e:
                logger_amp.error(f"NeuroPy parse error: {str(e)}")
    ...
    def _parse_payload(self, payload):
        ...
        while i < len(payload):
            ...
            if code == '02':  # 信号质量
                i += 1
                self.poor_signal = int(payload[i], 16)
                self._trigger_callback('poor_signal', self.poor_signal)

            elif code == '04':  # 注意力
                i += 1
                self.attention = int(payload[i], 16)
                self._trigger_callback('attention', self.attention)

            elif code == '05':  # 冥想度
                i += 1
                self.meditation = int(payload[i], 16)
                self._trigger_callback('meditation', self.meditation)

            elif code == '16':  # 眨眼强度
                i += 1
                self.blink_strength = int(payload[i], 16)
                self._trigger_callback('blink_strength', self.blink_strength)

            elif code == '80':  # 原始脑电
                i += 3  # 跳过长度字节和第一个数据字节
                self.raw_value = int(payload[i], 16) + int(payload[i - 1], 16) * 256
                if self.raw_value > 32768:
                    self.raw_value -= 65536
                self.raw_value_data.append(self.raw_value)
                self._trigger_callback('raw_value', self.raw_value)

            elif code == '83':  # 频带功率
                i += 1  # 跳过长度字节
                # 解析所有频带数据 (delta, theta, lowAlpha, highAlpha, lowBeta, highBeta, lowGamma, midGamma)
                bands = [
                ...
                for band in bands:
                    i += 3
                    value = (int(payload[i - 2], 16) << 16) + (int(payload[i - 1], 16) << 8) + int(payload[i], 16)
                    setattr(self, band, value)
                    self._trigger_callback(band, value)
            i += 1

    def _trigger_callback(self, name, value):
        """触发回调函数"""
    ...
    def set_callback(self, name, callback):
        """设置数据更新回调函数
        Args:
            name: 数据名称 (如 'attention', 'meditation' 等)
            callback: 回调函数
        """
        self.callbacks[name] = callback

    def recv(self):
        """实现基类方法 (NeuroPy使用回调机制，此方法返回None)"""
        return None
